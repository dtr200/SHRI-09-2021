...
const f = (amount, costsArray) => {
    ...
    /*
        ...
        сколько я смогу себе позволить maxNums. Эта переменная поможет двигаться по 
        уже сформированному числу из одинаковых минимальных индексов и ставить лучшие 
        значения сперва на 0 позицию, потом на 1 и т.д.
        ...
        Итого, сложность набить result наименьшими индексами O(n**2 log n + 2n log n + 3n).

        Потратить оставшиеся деньги - O(n**2)
        Итого, суммарная сложность алгоритма O(n**2 log n + n**2 + 2n log n + 3n),
        ...
        Суммарная сложность по памяти O(3n) или асимптотическая O(n).
    */
